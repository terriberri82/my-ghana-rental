...
export const getDashboard = async (req, res) => {
  ...
  for (const p of payments) {
    ...
    const bucket = months.find(
      (m) => m.month === p.paidAt.getMonth() && m.year === p.paidAt.getFullYear()
    );
    if (bucket) {
  ...
  res.status(200).json({
    ...
    recentPayments: recent.map((p) => ({
      ...
      method: p.method === "MOBILE_MONEY" ? "MoMo" : p.method === "CARD" ? "Card" : "Cash",
      date: p.paidAt.toLocaleDateString("en-GB", { day: "numeric", month: "short" }),
      amount: p.amount,
    })),
